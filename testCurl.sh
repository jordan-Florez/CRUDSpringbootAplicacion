...
curl -X POST http://localhost:8081/api/usuarios \ -H "Content-Type: application/json" \ -d '{"nombre": "Thiago", "password": "1234"}'

curl -X POST http://localhost:8081/api/notas -H "Content-Type: application/json" -d "{\"carpeta_id\": 1, \"id\": 1, \"contenido\": \"Este es el contenido de la nota\", \"titulo\": \"Nota de Prueba\"}"

# 1. Crear una nueva nota
curl -X POST $BASE_URL \
-H "Content-Type: application/json" 
-d "{\"titulo\":\"Mi Nota\", \"contenido\":\"Este es el contenido de la nota 1\"}"
echo -e "\n"
